...
class Preprocessor:
    ...
    def dropUnnecessaryColumns(self, data,columnNameList):
        """
                        ...
                        Description: This method drops the unwanted columns as discussed in EDA section.
                                ...
                                """
        self.data = data.drop(columns=columnNameList)
        self.logger_object.log(self.file_object, f'Successfully dropped the columns{columnNameList}')
        return self.data
    ...
    def encodeCategoricalValues(self,data):
     ...
     self.logger_object.log(self.file_object, 'Successful conversion of categorical data to numerical')

     numerical_features = ['age', 'TSH', 'T3', 'TT4', 'T4U', 'FTI']

     # Numerical cols are of object type as found while performing EDA which requires type conversion to float
     for features in data[numerical_features]:
         data[features] = data[features].astype(float)

     self.logger_object.log(self.file_object, 'Type conversion successful for numerical data')
    ...
    def impute_missing_values(self, data):
        ...
        try:
            ...
            # imputing the missing values of ['age', 'sex', 'TSH', 'TT4', 'FTI'] with KNN imputer
            self.new_array = imputer.fit_transform(self.data[['age', 'sex', 'TSH', 'TT4', 'FTI']])
            # rounding the value because KNNimputer returns value between 0 and 1, but we need either 0 or 1
            self.new_data = pd.DataFrame(data=np.round(self.new_array), columns=['age', 'sex', 'TSH', 'TT4', 'FTI'])
            self.logger_object.log(self.file_object,
                                   "Missing values imputed using KNNImputer for ['age', 'sex', 'TSH', 'TT4', 'FTI'] cols")

            #### Using median imputation to impute missing values on  T4U and T3 cols.
            cols = ['T4U', 'T3']
            for col in cols:
                self.data[col] = self.data[col].fillna(self.data[col].median())
            self.logger_object.log(self.file_object, "Missing values imputed using median  for 'T4U', 'T3' cols")

            # Dropping 'age', 'sex', 'TSH', 'TT4', 'FTI' cols from original data so that new_data can be concatenated with original data
            self.data.drop(columns=['age', 'sex', 'TSH', 'TT4', 'FTI'], inplace=True)
            self.logger_object.log(self.file_object, "'age', 'sex', 'TSH', 'TT4', 'FTI' columns dropped successfully")

            # Concatenating two dataframes
            self.data_concat = pd.concat([self.data, self.new_data], axis=1)
            self.logger_object.log(self.file_object, "New dataframe created  successfully")
